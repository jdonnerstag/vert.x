...
public class TestBase extends TestCase {
	...
	private static final int DEFAULT_TIMEOUT = Integer.parseInt(System
			.getProperty("vertx.test.timeout", "30"));
  ...
  // A single Vertx and VerticleManager for <b>ALL</b> tests
	...
	private final TestUtils tu = new TestUtils(vertx);
	private volatile Handler<Message<JsonObject>> handler;
	private final List<AssertHolder> failedAsserts = new ArrayList<>();
	private final List<String> startedApps = new CopyOnWriteArrayList<>();
				...
				try {
					...
					case EventFields.TRACE_EVENT:
						log.trace(message.body
								.getString(EventFields.TRACE_MESSAGE_FIELD));
						break;
					case EventFields.EXCEPTION_EVENT:
						failedAsserts
								.add(new AssertHolder(
										message.body
												.getString(EventFields.EXCEPTION_MESSAGE_FIELD),
										message.body
												.getString(EventFields.EXCEPTION_STACKTRACE_FIELD)));
						break;
					case EventFields.ASSERT_EVENT:
						boolean passed = EventFields.ASSERT_RESULT_VALUE_PASS
								.equals(message.body
										.getString(EventFields.ASSERT_RESULT_FIELD));
						if (passed) {
						} else {
							failedAsserts
									.add(new AssertHolder(
											message.body
													.getString(EventFields.ASSERT_MESSAGE_FIELD),
											message.body
													.getString(EventFields.ASSERT_STACKTRACE_FIELD)));
						}
						break;
	...
	protected String startApp(boolean worker, String main, JsonObject config)
			throws Exception {
		return startApp(worker, main, config, 1, true);
	}

	protected String startApp(boolean worker, String main, boolean await)
			throws Exception {
		return startApp(worker, main, null, 1, await);
	}

	protected String startApp(boolean worker, String main, int instances)
			throws Exception {
		return startApp(worker, main, null, instances, true);
	}

	protected String startApp(boolean worker, String main, JsonObject config,
			int instances, boolean await) throws Exception {
		...
		URL url;
		if (main.endsWith(".js") || main.endsWith(".rb")
				|| main.endsWith(".groovy") || main.endsWith(".py")) {
			...
			String surlroot = surl.substring(0,
					surl.length() - classDir.length());
    ...
    verticleManager.deployVerticle(worker, main, config, new URL[]{url}, instances, null, doneHandler);
		...
		if (!doneLatch.await(30, TimeUnit.SECONDS)) {
			throw new IllegalStateException(
					"Timedout waiting for apps to start");
		}
	...
	public String startMod(String modName, JsonObject config, int instances,
		...
		verticleManager
				.deployMod(modName, config, instances, null, doneHandler);
	...
	protected void waitEvent(int timeout, String eventName) {
		...
		if (!eventName.equals(message.getString("type"))) {
			throw new IllegalStateException("Expected event: " + eventName
					+ " got: " + message.getString(EventFields.TYPE_FIELD));
		}
	}
