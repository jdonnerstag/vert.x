...
public class HttpTestClient extends TestClientBase {
	...
	public void testClientChaining() {
		...
		HttpClientRequest req = client.put("someurl",
	...
	private void testSimpleRequest(final String method,
		...
		getRequest(specificMethod, method, "some-uri",
				new Handler<HttpClientResponse>() {
					@Override
					public void handle(HttpClientResponse resp) {
						tu.checkContext();
	...
	public void testAbsoluteURI() {
		testURIAndPath("http://localhost:8080/this/is/a/path/foo.html",
				"/this/is/a/path/foo.html");
	}
	...
	private void testParams(char delim) {
		...
		startServer(new Handler<HttpServerRequest>() {
			...
			public void handle(HttpServerRequest req) {
				...
				for (Map.Entry<String, String> entry : req.params().entrySet()) {
					tu.azzert(entry.getValue().equals(
		...
		getRequest(true, "GET", "some-uri/?" + query,
				new Handler<HttpClientResponse>() {
					@Override
					public void handle(HttpClientResponse resp) {
						tu.checkContext();
		...
		startServer(new Handler<HttpServerRequest>() {
			...
			public void handle(HttpServerRequest req) {
				...
				for (Map.Entry<String, String> entry : headers.entrySet()) {
					tu.azzert(entry.getValue().equals(
							req.headers().get(entry.getKey())));
				}
				req.response.end();
			}
		});

		HttpClientRequest req = getRequest(true, "GET", "some-uri",
				new Handler<HttpClientResponse>() {
					@Override
					public void handle(HttpClientResponse resp) {
						tu.checkContext();
  ...
  public void testRequestNotReceivedIfTimedout() {
	...
	public void testUseRequestAfterComplete() {
		...
		HttpClientRequest req = getRequest(true, "POST", "some-uri",
				new Handler<HttpClientResponse>() {
					@Override
					public void handle(HttpClientResponse resp) {
						tu.checkContext();
	...
	private void writeChunk(final int remaining, final int chunkSize,
			final HttpClientRequest req, final Buffer totBuffer) {
		if (remaining > 0) {
			Buffer b = TestUtils.generateRandomBuffer(chunkSize);
	...
	private void testRequestBodyWriteString(boolean chunked,
			boolean waitCompletion, String encoding) {
		...
		final HttpClientRequest req = getRequest(true, "POST", "some-uri",
				new Handler<HttpClientResponse>() {
					@Override
					public void handle(HttpClientResponse resp) {
						tu.checkContext();
	...
	private void testResponseHeaders(final boolean individually) {
		...
		startServer(new Handler<HttpServerRequest>() {
			...
			public void handle(HttpServerRequest req) {
				...
				if (individually) {
					for (Map.Entry<String, String> header : headers.entrySet()) {
						req.response.headers().put(header.getKey(),
								header.getValue());
					}
				} else {
			...
			public void handle(HttpServerRequest req) {
				...
				if (individually) {
					for (Map.Entry<String, String> header : trailers.entrySet()) {
						req.response.trailers().put(header.getKey(),
								header.getValue());
					}
				} else {
				...
				new Handler<HttpClientResponse>() {
					...
					public void handle(final HttpClientResponse resp) {
						...
						resp.endHandler(new SimpleHandler() {
							@Override
							public void handle() {
								tu.azzert(resp.trailers().size() == trailers
										.size());
								for (Map.Entry<String, String> entry : trailers
										.entrySet()) {
									tu.azzert(entry.getValue()
											.equals(resp.trailers().get(
													entry.getKey())));
								}
								tu.testComplete();
							}
		...
		startServer(new Handler<HttpServerRequest>() {
			...
			public void handle(HttpServerRequest req) {
				...
				if (chunked) {
					...
					req.response.headers().put("Content-Length",
							numWrites * chunkSize);
				}
				if (waitCompletion) {
	...
	private void writeChunk(final int remaining, final int chunkSize,
			final HttpServerResponse resp, final Buffer totBuffer) {
		if (remaining > 0) {
			Buffer b = TestUtils.generateRandomBuffer(chunkSize);
			totBuffer.appendBuffer(b);
	...
	private void testResponseBodyWriteString(final boolean chunked,
			final boolean waitCompletion, final String encoding) {
		...
		final HttpClientRequest req = getRequest(true, "GET", "some-uri",
				new Handler<HttpClientResponse>() {
					@Override
					public void handle(HttpClientResponse resp) {
						tu.checkContext();
	...
	public void testPipelining() {
			...
			public void handle(final HttpServerRequest req) {
				...
				req.bodyHandler(new Handler<Buffer>() {
					...
					public void handle(final Buffer buff) {
						tu.azzert(("This is content " + theCount).equals(buff
								.toString()), buff.toString());
						// We write the response back after a random time to
						// increase the chances of responses written in the
						// wrong order if we didn't implement pipelining
						// correctly
						vertx.setTimer((long) (10 * Math.random()),
								new Handler<Long>() {
									@Override
									public void handle(Long timerID) {
										req.response.headers().put("count",
												String.valueOf(theCount));
		...
		for (int count = 0; count < requests; count++) {
			final int theCount = count;
			HttpClientRequest req = client.request("POST", "some-uri",
					new Handler<HttpClientResponse>() {
						@Override
						public void handle(final HttpClientResponse response) {
							tu.azzert(theCount == Integer.parseInt(response
									.headers().get("count")), theCount + ":"
									+ response.headers().get("count"));
							response.bodyHandler(new Handler<Buffer>() {
								@Override
								public void handle(Buffer buff) {
									tu.azzert(("This is content " + theCount)
	...
	public void testSendFile() throws Exception {
		...
		client.getNow("some-uri", new Handler<HttpClientResponse>() {
			...
			public void handle(final HttpClientResponse response) {
				...
				tu.azzert(file.length() == Long.valueOf(response.headers().get(
						"content-length")));
				tu.azzert("text/html".equals(response.headers().get(
						"content-type")));
				response.bodyHandler(new Handler<Buffer>() {
					@Override
					public void handle(Buffer buff) {
						String buf = buff.toString();
						tu.azzert(content.equals(buf));
						file.delete();
						tu.testComplete();
					}
				});
	...
	public void testSendFileOverrideHeaders() throws Exception {
		...
		client.getNow("some-uri", new Handler<HttpClientResponse>() {
			...
			public void handle(final HttpClientResponse response) {
				...
				tu.azzert(file.length() == Long.valueOf(response.headers().get(
						"content-length")));
				tu.azzert("wibble".equals(response.headers()
						.get("content-type")));
				response.bodyHandler(new Handler<Buffer>() {
					@Override
					public void handle(Buffer buff) {
	...
	public void test100ContinueDefault() throws Exception {
		...
		final HttpClientRequest req = client.put("someurl",
				new Handler<HttpClientResponse>() {
					@Override
					public void handle(HttpClientResponse resp) {
						resp.endHandler(new SimpleHandler() {
	...
	public void testClientDrainHandler() {
		final HttpClientRequest req = client.get("someurl",
				new Handler<HttpClientResponse>() {
					@Override
					public void handle(HttpClientResponse resp) {
					}
		...
		final HttpClientRequest req = client.get("someurl",
				new Handler<HttpClientResponse>() {
					...
					public void handle(final HttpClientResponse resp) {
						...
						vertx.eventBus().registerHandler("client_resume",
								resumeHandler);
						resp.endHandler(new SimpleHandler() {
							@Override
							public void handle() {
	...
	private void testPooling(final boolean keepAlive) throws Exception {
		...
		for (int i = 0; i < numGets; i++) {
			...
			HttpClientRequest req = client.get(path,
					new Handler<HttpClientResponse>() {
						@Override
						public void handle(final HttpClientResponse response) {
							tu.azzert(response.statusCode == 200);
  ...
  public void testConnectionErrorsGetReportedToRequest() {
	...
	private void tls() {
		TLSTestParams params = TLSTestParams.deserialize(vertx.sharedData()
				.<String, byte[]> getMap("TLSTest").get("params"));
		...
		if (params.clientTrust) {
			client.setTrustStorePath(
					"./src/test/keystores/client-truststore.jks")
		...
		HttpClientRequest req = client.get("someurl",
				new Handler<HttpClientResponse>() {
					@Override
					public void handle(final HttpClientResponse response) {
						tu.checkContext();
...
//    });
	...
	public void testSharedServersMultipleInstances1() {
		...
		final int numRequests = vertx.sharedData()
				.<String, Integer> getMap("params").get("numRequests");
	...
	private HttpClientRequest getRequest(boolean specificMethod, String method,
			String uri, Handler<HttpClientResponse> responseHandler) {
	...
	private Map<String, String> genMap(int num) {
		...
		for (int i = 0; i < num; i++) {
			String key;
			do {
				key = TestUtils.randomAlphaString(
						1 + (int) ((19) * Math.random())).toLowerCase();
			} while (map.containsKey(key));
			map.put(key, TestUtils.randomAlphaString(1 + (int) ((19) * Math
					.random())));
		}
		return map;
	}
