...
public class TestUtils {
  ...
  public TestUtils(final Vertx vertx) {
	...
	public void azzert(boolean result, String message) {
		JsonObject jsonObject = new JsonObject().putString(
				EventFields.TYPE_FIELD, EventFields.ASSERT_EVENT).putString(
				EventFields.ASSERT_RESULT_FIELD,
				result ? EventFields.ASSERT_RESULT_VALUE_PASS
						: EventFields.ASSERT_RESULT_VALUE_FAIL);
		if (message != null) {
			jsonObject.putString(EventFields.ASSERT_MESSAGE_FIELD, message);
		}
		if (!result) {
			jsonObject.putString(EventFields.ASSERT_STACKTRACE_FIELD,
					getStackTrace(new Exception()));
		}
		sendMessage(jsonObject);
		if (!result) {
			// We throw a RTE not an AssertFailedError since Rhino cannot catch
			// errors in
			// test_utils.js
	...
	public void testComplete() {
		JsonObject jsonObject = new JsonObject().putString(
				EventFields.TYPE_FIELD, EventFields.TEST_COMPLETE_EVENT)
	...
	public void startTest(String testName) {
		JsonObject jsonObject = new JsonObject().putString(
				EventFields.TYPE_FIELD, EventFields.START_TEST_EVENT)
	...
	public void exception(Throwable t, String message) {
		JsonObject jsonObject = new JsonObject()
				.putString(EventFields.TYPE_FIELD, EventFields.EXCEPTION_EVENT)
				.putString(EventFields.EXCEPTION_MESSAGE_FIELD, message)
				.putString(EventFields.EXCEPTION_STACKTRACE_FIELD,
						getStackTrace(t));
		sendMessage(jsonObject);
	}
	...
	public void trace(String message) {
		JsonObject jsonObject = new JsonObject().putString(
				EventFields.TYPE_FIELD, EventFields.TRACE_EVENT).putString(
				EventFields.TRACE_MESSAGE_FIELD, message);
		sendMessage(jsonObject);
	}
	...
	public void register(final String testName, final Handler<Void> handler) {
		Handler<Message<JsonObject>> h = new Handler<Message<JsonObject>>() {
			...
			public void handle(Message<JsonObject> msg) {
				if (EventFields.START_TEST_EVENT.equals(msg.body
						.getString(EventFields.TYPE_FIELD))
						&& testName.equals(msg.body
								.getString(EventFields.START_TEST_NAME_FIELD))) {
					handler.handle(null);
				}
			}
		};
	...
	public void unregisterAll() {
		for (Handler<Message<JsonObject>> handler : handlers.values()) {
			vertx.eventBus()
					.unregisterHandler(TestBase.EVENTS_ADDRESS, handler);
		}
		handlers.clear();
	}
	...
	public void sendEvent(String eventName) {
		JsonObject msg = new JsonObject().putString(EventFields.TYPE_FIELD,
				eventName);
		sendMessage(msg);
	}
	...
	public static byte[] generateRandomByteArray(int length, boolean avoid,
			byte avoidByte) {
		byte[] line = new byte[length];
		for (int i = 0; i < length; i++) {
			// Choose a random byte - if we're generating delimited lines then
			// make sure we don't
	...
	public static Buffer generateRandomBuffer(int length, boolean avoid,
			byte avoidByte) {
	...
	public static String randomUnicodeString(int length) {
		...
		for (int i = 0; i < length; i++) {
			char c;
			do {
				c = (char) r.nextInt(0xFFFF);
			} while ((c >= 0xFFFE && c <= 0xFFFF)
					|| (c >= 0xD800 && c <= 0xDFFF)); // Illegal chars
			builder.append(c);
		}
	...
	public void checkContext() {
		if (context == null) {
			throw new IllegalStateException(
					"Don't call checkContext if utils were created with a null context");
		}
		azzert(th == Thread.currentThread(), "Expected:" + th + " Actual:"
    azzert(context.equals(vertx.getContext()), "Expected:" + context + " Actual:" + vertx.getContext());
		azzert(context.equals(Context.getContext()), "Expected:" + context
